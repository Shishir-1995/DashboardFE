...
class AuthImp {
  public async login(payload: LoginReqDto) {
    const { data } = await httpClient.post<LoginResDto>(
      "v1/auth/user?session=login",
      payload
    );
  ...
  public async getStudentProfile(): Promise<
    ProfileSuccessDto<StudentProfileDto>
  > {
  ...
  public async getIaProfile(): Promise<ProfileSuccessDto<IAProfileDto>> {
    const { data } = await httpClient.post<ProfileSuccessDto<IAProfileDto>>(
      "auth/profile"
    );
    return data;
  }
